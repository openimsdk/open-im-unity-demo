...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed class ResourceEditorController
    {
        ...
        public SourceFolder LuaSourceAssetRoot = new SourceFolder("Product/Lua", null);
        ...
        public static string LuaRootPath = Utility.Path.GetRegularPath(Path.Combine(System.Environment.CurrentDirectory, "Product/Lua"));
        public void ScanLuaSource()
        {
            LuaSourceAssetRoot.Clear();
            DirectoryInfo folder = new DirectoryInfo(LuaRootPath);
            FileInfo[] files = folder.GetFiles("*.lua", SearchOption.AllDirectories);
            for (int i = 0; i < files.Length; i++)
            {
                var file = files[i];
                var fullPath = Utility.Path.GetRegularPath(file.FullName);
                var guid = fullPath.Substring(System.Environment.CurrentDirectory.Length + 1);
                var path = fullPath.Substring(LuaRootPath.Length + 1);
                string[] splitedPath = path.Split('/');
                byte[] bytes = File.ReadAllBytes(fullPath);
                SourceFolder luaFolder = LuaSourceAssetRoot;
                for (int j = 0; j < splitedPath.Length - 1; j++)
                {
                    SourceFolder subFolder = luaFolder.GetFolder(splitedPath[j]);
                    luaFolder = subFolder == null ? luaFolder.AddFolder(splitedPath[j]) : subFolder;
                }
                SourceAsset asset = luaFolder.AddAsset(guid,path,Path.GetFileNameWithoutExtension(path));
                asset.Icon = AssetDatabase.LoadAssetAtPath<Texture>("Assets/XLua/Editor/logo.png");
                m_SourceAssets.Add(asset.Guid, asset);
            }
        }
