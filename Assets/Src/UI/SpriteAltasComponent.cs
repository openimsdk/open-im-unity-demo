...
namespace Dawn
{
    public class SpriteAltasComponent : GameFrameworkComponent
    {
        ...
        {
            if (spriteAltasDic.ContainsKey(name))
            {
                return spriteAltasDic[name];
            }
            return null;
        }

        public Sprite GetSprite(string atlas, string spritePath)
        {
            SpriteAtlas sa = null;
            ...
            if (suc)
            {
                ...
                if (sprite == null)
                {
                    Debug.LogWarning(string.Format("{0}.spritealtas no Sprite -> {1}", atlas, spritePath));
                }
                return sprite;
            }
            else
            {
                Debug.LogWarning("Cant load SpriteAtlas " + atlas);
            }
            return null;
        }
